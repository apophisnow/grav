...
void keyPressed() {
  if (key == 's' || key == 'S') {
    if (!serialMode) {
      ...
      int[] starData = new int[2];
      float[] inByteA = new float[2];
      float errRatio;
      if (!(mouseButton == RIGHT)) {
        // get the ASCII string:
        myString = new String(int(random(1, signalMax))+","+int(random(1, signalMax))+'\n');
        //println(myString);
        ...
        if (inByteA[0] >= 1 || inByteA[1] >= 1) {
          ...
          stars[0][xPos] = new Star(c1, starData[0], starData[0], starData[0]);
          stars[1][xPos] = new Star(c2, starData[1], starData[1], starData[1]);
          xPos++;
        }
...
void orbit(){
  if (xPos >= width) {
    ...
    PVector s1location = new PVector(map(montecarlo(),0,1,mouseX+1, width), mouseY);
